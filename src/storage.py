...
class StorageService:
    ...
    def init_db(self):
        ...
        """)
        # Goals, SubGoals, LongTerm, Enemies - залишаються без змін з минулого разу...
        cursor.execute("""
        ...
        # 1. Бібліотека предметів (ОНОВЛЕНО)
        cursor.execute("""
    ...
    def seed_items(self):
        ...
        if cursor.fetchone()[0] == 0:
            # ТЕСТОВИЙ НАБІР (+1 до статів згідно ТЗ)
            items = [
                # Голова -> +INT
                Item("Капелюх Учня", ItemType.ARMOR, EquipmentSlot.HEAD, bonus_int=1, price=10,
                     image_path="hat_test.png"),
                # Тіло -> +HP, +DEF
                Item("Ганчір'я", ItemType.ARMOR, EquipmentSlot.BODY, bonus_vit=1, bonus_def=1, price=10,
                     image_path="cloth_test.png"),
                # Ноги -> +HP, +DEF
                Item("Штани Новачка", ItemType.ARMOR, EquipmentSlot.LEGS, bonus_vit=1, bonus_def=1, price=10,
                     image_path="pants_test.png"),
                # Взуття -> +DEX
                Item("Старі Черевики", ItemType.ARMOR, EquipmentSlot.FEET, bonus_dex=1, price=10,
                     image_path="boots_test.png"),
                # Руки -> +STR
                Item("Рукавиці", ItemType.ARMOR, EquipmentSlot.HANDS, bonus_str=1, price=10,
                     image_path="gloves_test.png"),

                # Зброя
                Item("Іржавий Меч", ItemType.WEAPON, EquipmentSlot.MAIN_HAND,
                     weapon_class=WeaponClass.SWORD, base_dmg=2, bonus_str=1, price=15, image_path="sword_test.png"),
                Item("Старий Лук", ItemType.WEAPON, EquipmentSlot.MAIN_HAND,
                     weapon_class=WeaponClass.BOW, base_dmg=2, bonus_dex=1, price=15, image_path="bow_test.png"),
                Item("Гілка (Посох)", ItemType.WEAPON, EquipmentSlot.MAIN_HAND,
                     weapon_class=WeaponClass.STAFF, base_dmg=1, bonus_int=1, damage_type=DamageType.MAGICAL, price=15,
                     image_path="staff_test.png"),
                Item("Ножик", ItemType.WEAPON, EquipmentSlot.MAIN_HAND,
                     weapon_class=WeaponClass.DAGGER, base_dmg=1, bonus_dex=1, price=15, image_path="dagger_test.png"),

                # Щит
                Item("Кришка від бочки", ItemType.WEAPON, EquipmentSlot.OFF_HAND,
                     weapon_class=WeaponClass.SHIELD, bonus_def=1, price=10, image_path="shield_test.png"),
            ]
            ...
            for item in items:
                ...
                """, (
                    str(item.id), item.name, item.item_type.value, item.slot.value if item.slot else None,
    ...
    def get_inventory(self, hero_id: str) -> List[InventoryItem]:
        ...
        inventory = []
        for row in rows:
            # row[0]=inv_id, row[1]=equipped, row[2]=lib_id, row[3]=name ...
    ...
    def get_all_library_items(self) -> List[Item]:
        ...
        items = []
        for row in rows:
            # Парсинг аналогічно get_inventory, тільки без полів інвентаря
    ...
    # ... (ТУТ ТРЕБА ВСТАВИТИ ВСІ СТАРІ МЕТОДИ create_hero, update_hero, save_goal, load_goals...) ...
    # Я їх не дублюю, бо вони не змінились, просто не забудьте їх додати.
    ...
    def load_enemy(self, hero_id: str) -> Optional[Enemy]:
        ...
        row = cursor.fetchone()
        conn.close()
        if row: return Enemy(id=uuid.UUID(row[1]), name=row[2], rarity=EnemyRarity(row[3]), level=row[4],
