...
from .models import Goal, SubGoal, Hero, Difficulty, LongTermGoal, HeroClass, Gender, Enemy, EnemyRarity, DamageType
...
class StorageService:
    ...
    def _map_row_to_hero(self, row) -> Hero:
        return Hero(
            ...
            stat_points=row[12], str_stat=row[13], int_stat=row[14], dex_stat=row[15], vit_stat=row[16], def_stat=row[17],
    ...
    def save_goal(self, goal: Goal, hero_id: str):
        ...
        try:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (str(goal.id), hero_id, goal.title, goal.description, goal.deadline.isoformat(), goal.difficulty.value, goal.created_at.isoformat(), 1 if goal.is_completed else 0, 1 if goal.penalty_applied else 0))
            cursor.execute("DELETE FROM sub_goals WHERE goal_id = ?", (str(goal.id),))
            for sub in goal.subgoals:
                cursor.execute("INSERT INTO sub_goals (id, goal_id, title, is_completed) VALUES (?, ?, ?, ?)", (str(sub.id), str(goal.id), sub.title, 1 if sub.is_completed else 0))
            conn.commit()
        finally:
            conn.close()
    ...
    def load_goals(self, hero_id: str) -> List[Goal]:
        ...
        cursor = conn.cursor()
        goals_list = []
        cursor.execute("SELECT id, title, description, deadline, difficulty, created_at, is_completed, penalty_applied FROM goals WHERE hero_id = ?", (hero_id,))
        rows = cursor.fetchall()
        for row in rows:
            g_id, title, desc, dl_str, diff_val, ca_str, is_comp, is_penalized = row
            goal = Goal(title=title, description=desc, deadline=datetime.fromisoformat(dl_str), difficulty=Difficulty(diff_val))
    ...
    def save_long_term_goal(self, goal: LongTermGoal, hero_id: str):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (str(goal.id), hero_id, goal.title, goal.description, goal.total_days, goal.start_date.isoformat(), goal.time_frame, goal.current_day, goal.checked_days, goal.missed_days, 1 if goal.is_completed else 0, goal.daily_state, last_update))
        conn.commit()
        conn.close()
    ...
    def load_long_term_goals(self, hero_id: str) -> List[LongTermGoal]:
        ...
        goals = []
        for row in rows:
            g = LongTermGoal(title=row[2], description=row[3], total_days=row[4], start_date=datetime.fromisoformat(row[5]), time_frame=row[6])
    ...
    def save_enemy(self, enemy: Enemy, hero_id: str):
        ...
        """, (
            ...
            enemy.current_hp, enemy.max_hp, enemy.damage, enemy.damage_type.value, # Зберігаємо тип як рядок
    ...
    def load_enemy(self, hero_id: str) -> Optional[Enemy]:
        ...
        if row:
            return Enemy(
                ...
                max_hp=row[6],
                damage=row[7],
                damage_type=DamageType(row[8]), # Відновлюємо Enum
                reward_xp=row[9],
                reward_gold=row[10],
                drop_chance=row[11],
                image_path=row[12]
